...
class CreateEnvironment(object):
    ...
    def __init__(self, map_topic, thresholds=1, height=1.0):
        ...
        self.path_to_package = rospack.get_path('robot_nav')
        self.name = rospy.get_param("~name", "regus")
    ...
    def map_callback(self, msg):
        ...
        # Export DAE
        export_dir = self.path_to_package + "/models/" + self.name + "/"

        with open(export_dir + self.name + '.dae', 'w') as f:
            f.write(trimesh.exchange.dae.export_collada(mesh).decode()) 

        # Shut down the program
        rospy.loginfo(export_dir)
